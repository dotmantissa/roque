...
/**
 * The full faucet, given its own room. The wallet panel on the home screen has a
 * compact version of this, but here every token gets a card: what you hold, how
 * many pulls you have left drawn as little pips, and one button to claim more.
 * The header carries the single signature that tops up everything at once. It is
 * all testnet play money and every pull is signed by the person, so there is
 * nothing to gate and nothing to lose; grab what you need and go trade.
 */
...
import { walletBalances, faucetClaimsRemaining, claimFaucet, claimAllFaucets } from "@/lib/chain";
...
export default function FaucetPage() {
  ...
  const balances = usePoll(address ? () => walletBalances(address) : null, 20_000, [address]);
    ...
    <>
      ...
      <main className="page">
        <div className="page-inner">
          <Link href="/" className="faucet-back">
            <ArrowLeft size={15} />
            Back to trading
          </Link>
          ...
          <section className="faucet-hero">
            <div>
              ...
              <p className="faucet-sub">
                Ten test tokens, each pull worth about a thousand dollars, priced off the
                same Chainlink feeds the pools were seeded at. Grab a few and go put them to
                work. Gas is plain Sepolia ETH, so keep a little of that around too.
              </p>
            </div>
            {wallet.connected ? (
